...
├── ROADMAP.md                # 工作項目編號（P0–P8）與模組總表——唯一定義處
│
...
│   ├── layout_section.png            # 按比例軸對稱半剖圖（P8，由 generate_layout.generate_section() 產生）
│   └── layout_architecture.png       # 四物理域架構圖（P8，由 generate_layout.generate_architecture() 產生）
│
├── solenoid_model/                              # 核心程式套件
│   ├── __init__.py
│   ├── params.py           # ValveParams dataclass — 所有可調參數的定義
│   ├── baseline.py         # BASELINE_PARAMS — 預先驗證過的基準案例數值
│   ├── magnetization.py    # 單值 B-H 磁化曲線（P8）：AnalyticBH（Fröhlich-Kennelly 解析式，
│   │                       #   零新增自由度，重用 mu_r_core/B_sat）／TabulatedBH（量測點，
│   │                       #   分段線性、鉗位不外推）／from_params；刻意不含磁滯
│   ├── magnetics.py        # 磁路：reluctance/inductance/coenergy力/飽和標記（只吃物理氣隙 gap）；
│   │                       #   `solve_flux_density`（P8，對 B 二分求解 MMF 平衡）+
│   │                       #   `flux_density`/`magnetic_force_closing`/`saturation_check` 的
│   │                       #   `mag=None` 第五旋鈕（線性路徑 bit-level 不變）
...
│   ├── layout.py           # 實體佈局尺寸鏈（P8）：yoke_area／shell_wall_thickness（磁性/
│   │                       #   結構/製造三下限取大者）／spring_geometry（封閉解）／
│   │                       #   armature_thickness（由 m_arm 反解）+ armature_mass_consistency
│   │                       #   （一致性檢查）／axial_stack／envelope；LAYOUT_EMPIRICAL（封裝
│   │                       #   經驗值集中宣告）＋ SPRING_DESIGN（規格 §6 彈簧設計點，DERIVED）；
│   │                       #   純代數，不 import dynamics/magnetics/fluid
│   ├── app.py              # Streamlit 五頁籤介面：compute_*（純函式：compute_result／
│   │                       #   compute_opening_bounce／compute_closing／compute_bounce_spike／
│   │                       #   compute_layout）+ render_*_tab（render_opening_tab／
│   │                       #   render_closing_tab／render_limits_tab／render_sealing_tab／
│   │                       #   render_layout_tab）+ main()（側欄組裝
│   │                       #   ValveParams、st.tabs 分派五頁：開啟動態｜關閉/續流｜
│   │                       #   極限掃描 L1–L3｜密封/壽命 L4–L5｜實體佈局，P7/P8）
│   │                       #   執行方式：streamlit run solenoid_model/app.py
│   │
│   ├── tests/               # pytest 測試（同時也是分層驗證）
│   │   ├── test_params.py
│   │   ├── test_magnetization.py       # AnalyticBH/TabulatedBH（P8）：B(H)/H(B) 互逆、
│   │   │                                #   漸近 B_sat、鉗位不外推、from_params
│   │   ├── test_magnetics.py
│   │   ├── test_magnetics_bh.py        # solve_flux_density（P8）：mag=None 與線性結果逐位元
│   │   │                                #   相同、飽和模式下 MMF 平衡解回代成立
│   │   ├── test_drive_bh.py            # drive.py 的 mag 引數（P8）：飽和模式下電流反解
│   │   │                                #   仍與力平衡回代一致
│   │   ├── test_dynamics_rl.py
...
│   │   ├── test_layout.py              # yoke_area/shell_wall_thickness/spring_geometry/
│   │   │                                #   armature_thickness/axial_stack/envelope（P8）
│   │   ├── test_layout_report.py       # generate_layout 兩張圖產出 + rcParams 不污染全域（P8）
│   │   └── test_app_layout.py          # compute_layout/render_layout_tab（P8）
│   │
│   └── report/               # 基準案例報告腳本與輸出圖
...
│       ├── generate_layout.py          # generate_section()／generate_architecture()：按比例
│       │                               #   剖面圖 + 四域架構圖（P8），寫 docs/spec/layout_*.png
...
第 2 層｜物理函式層 magnetics.py（磁路）｜magnetization.py（B-H 曲線，P8）｜fluid.py（流量/流體力）｜
                  dynamics 元件函式｜sizing.py／winding.py／layout.py（純代數分支，不進 ODE）
...
密封／回彈沿用同一原則：`seat=None`（預設）維持既有硬鉗制止擋行為，對現有呼叫者 bit-level 相同；傳入 `sealing.SeatPair` 才切換到回彈積分（`dynamics.simulate_opening`/`simulate_closing` 的 `seat` 參數，事件點 `v → −e·v` 後續積分至 `|v| < v_min` 或達 `n_max`）。連同 `medium`/`cond` 與 `T_coil`，`seat` 是第四個正交模式旋鈕。
...
`medium`/`cond`、`T_coil` 與 `seat`，`mag` 是**第五個正交模式旋鈕**：五者互相獨立、可任意組合，庫層本身不預設 `None` 以外的任何模式，「預設開啟」與否的選擇權一律留給第 4 層。
